...
5. Получить список ценных бумаг, отправив GET запрос на `http://"host":"port"/security/list` с параметрами:
...
23. Получить страницу для формирования запроса получения сводной таблицы из тегов:
    ...
    - open
    - close
отправить GET запрос` http://"host":"port"/pivot_table`.

24. Получить сводную таблицу - отправить GET запрос на `http://"host":"port"/pivot_table/list `
с параметрами:
    - limit - количество выводимых элементов (обязательный);
    - offset - смещение от начала списка (обязательный);
